<template>
  <div class="process-page">
    ...
    <nav class="navbar">
      <div class="nav-brand" @click="goHome">StrangeVerse</div>
      
      
      <div class="nav-center">
        <div class="step-badge">STEP 01</div>
        <div class="step-name">{{ t('processGraph.stepName') }}</div>
      </div>
    ...
    <div class="main-content">
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          <div class="header-left">
            ...
            <span class="header-title">{{ t('processGraph.liveGraph') }}</span>
          </div>
          <div class="header-right">
            <template v-if="graphData">
              <span class="stat-item">{{ graphData.node_count || graphData.nodes?.length || 0 }} {{ t('processGraph.nodes') }}</span>
              <span class="stat-divider">|</span>
              <span class="stat-item">{{ graphData.edge_count || graphData.edges?.length || 0 }} {{ t('processGraph.edges') }}</span>
              <span class="stat-divider">|</span>
            </template>
            <div class="action-buttons">
                <button class="action-btn" @click="refreshGraph" :disabled="graphLoading" :title="t('processGraph.refreshTitle')">
                  <span class="icon-refresh" :class="{ 'spinning': graphLoading }">↻</span>
                </button>
                <button class="action-btn" @click="toggleFullScreen" :title="isFullScreen ? t('processGraph.fullscreenExit') : t('processGraph.fullscreenEnter')">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">{{ t('processGraph.loadingGraph') }}</p>
          </div>
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-text">{{ t('processGraph.waitOntology') }}</p>
            <p class="waiting-hint">{{ t('processGraph.waitOntologyHint') }}</p>
          </div>
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-text">{{ t('processGraph.buildingGraph') }}</p>
            <p class="waiting-hint">{{ t('processGraph.dataSoon') }}</p>
          </div>
      ...
      <div class="right-panel" :class="{ 'hidden': isFullScreen }">
        <div class="panel-header dark-header">
          ...
          <span class="header-title">{{ t('processGraph.buildFlow') }}</span>
        </div>
        ...
        <div class="process-content">
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ t('processGraph.ontologyGen') }}</div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">{{ t('processGraph.apiLabel') }}</div>
                <div class="detail-content">
                  {{ t('processGraph.apiDescOntology') }}
                </div>
              </div>
              
              
              <div class="detail-section" v-if="ontologyProgress && currentPhase === 0">
                <div class="detail-label">{{ t('processGraph.genProgress') }}</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">{{ t('processGraph.entityTypes', { count: projectData.ontology.entity_types?.length || 0 }) }}</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">{{ t('processGraph.relationTypes', { count: projectData.ontology.relation_types?.length || 0 }) }}</div>
                <div class="relation-list">
                  ...
                  <div v-if="(projectData.ontology.relation_types?.length || 0) > 5" class="relation-more">
                    {{ t('processGraph.moreRelations', { count: projectData.ontology.relation_types.length - 5 }) }}
                  </div>
                </div>
              </div>
              ...
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 && !ontologyProgress">
                <div class="waiting-hint">{{ t('processGraph.waitOntologyDots') }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ t('processGraph.graphBuildPhase') }}</div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">{{ t('processGraph.apiLabel') }}</div>
                <div class="detail-content">
                  {{ t('processGraph.apiDescGraph') }}
                </div>
              </div>
              
              
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">{{ t('processGraph.waitOntologyDone') }}</div>
              </div>
              
              
              <div class="detail-section" v-if="buildProgress && currentPhase >= 1">
                <div class="detail-label">{{ t('processGraph.buildProgress') }}</div>
              ...
              <div class="detail-section" v-if="graphData">
                <div class="detail-label">{{ t('processGraph.buildResult') }}</div>
                <div class="build-result">
                  <div class="result-item">
                    <span class="result-value">{{ graphData.node_count }}</span>
                    <span class="result-label">{{ t('processGraph.entityNodes') }}</span>
                  </div>
                  <div class="result-item">
                    <span class="result-value">{{ graphData.edge_count }}</span>
                    <span class="result-label">{{ t('processGraph.relationEdges') }}</span>
                  </div>
                  <div class="result-item">
                    <span class="result-value">{{ entityTypes.length }}</span>
                    <span class="result-label">{{ t('processGraph.schemaTypes') }}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ t('processGraph.buildDone') }}</div>
                <div class="phase-api">{{ t('processGraph.readyNext') }}</div>
              </div>
        ...
        <div class="project-panel">
          <div class="project-header">
            ...
            <span class="project-title">{{ t('processGraph.projectInfo') }}</span>
          </div>
          <div class="project-details" v-if="projectData">
            <div class="project-item">
              <span class="item-label">{{ t('processGraph.projectName') }}</span>
              <span class="item-value">{{ projectData.name }}</span>
            </div>
            <div class="project-item">
              <span class="item-label">{{ t('processGraph.projectId') }}</span>
              <span class="item-value code">{{ projectData.project_id }}</span>
            </div>
            <div class="project-item" v-if="projectData.graph_id">
              <span class="item-label">{{ t('processGraph.graphId') }}</span>
              <span class="item-value code">{{ projectData.graph_id }}</span>
            </div>
            <div class="project-item">
              <span class="item-label">{{ t('processGraph.simRequirement') }}</span>
...
const statusText = computed(() => {
  if (error.value) return t('processGraph.statusBuildFailed')
  if (currentPhase.value >= 2) return t('processGraph.statusBuildComplete')
  if (currentPhase.value === 1) return t('processGraph.statusGraphBuilding')
  if (currentPhase.value === 0) return t('processGraph.statusOntologyGen')
  return t('processGraph.statusInit')
})
...
const getPhaseStatusText = (phase) => {
  if (currentPhase.value > phase) return t('processGraph.phaseDone')
  if (currentPhase.value === phase) {
    if (phase === 1 && buildProgress.value) {
      return `${buildProgress.value.progress}%`
    }
    return t('processGraph.phaseActive')
  }
  return t('processGraph.phasePending')
}
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = t('processGraph.errNoPendingFiles')
    loading.value = false
    return
  }
  ...
  try {
    ...
    ontologyProgress.value = { message: t('processGraph.msgUploadAnalyzing') }
    ...
    if (response.success) {
      ...
      error.value = response.error || t('processGraph.errOntologyFailed')
    }
  } catch (err) {
    console.error('Handle new project error:', err)
    error.value = t('processGraph.errInitFailed') + ': ' + (err.message || t('processGraph.unknownError'))
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || t('processGraph.errLoadProject')
    }
  } catch (err) {
    console.error('Load project error:', err)
    error.value = t('processGraph.errLoadProject') + ': ' + (err.message || t('processGraph.unknownError'))
  } finally {
    loading.value = false
  }
}
...
const updatePhaseByStatus = (status) => {
  switch (status) {
    ...
    case 'graph_completed':
      ...
      break
    case 'failed':
      error.value = projectData.value?.error || t('processGraph.errProcessing')
      break
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    buildProgress.value = {
      progress: 0,
      message: t('processGraph.msgStartBuild')
    }
    
    const response = await buildGraph({ project_id: currentProjectId.value })
    
    if (response.success) {
      buildProgress.value.message = t('processGraph.msgTaskStarted')
      ...
      error.value = response.error || t('processGraph.errStartBuild')
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = t('processGraph.errStartBuild') + ': ' + (err.message || t('processGraph.unknownError'))
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      buildProgress.value = {
        ...
        message: task.message || t('processGraph.msgProcessing')
      }
      ...
      if (task.status === 'completed') {
        ...
        buildProgress.value = {
          progress: 100,
          message: t('processGraph.msgBuildCompleteLoading')
        }
        ...
        stopPolling()
        stopGraphPolling()
        error.value = t('processGraph.errGraphBuildFailed') + ': ' + (task.error || t('processGraph.unknownError'))
        buildProgress.value = null
      }
    }
  } catch (err) {
...
const renderGraph = () => {
  ...
  if (nodesData.length === 0) {
    ...
    svg.append('text')
      ...
      .attr('fill', '#999')
      .text(t('processGraph.graphWaitingData'))
    return
  }
  ...
  const nodes = nodesData.map(n => ({
    id: n.uuid,
    name: n.name || t('processGraph.unnamed'),
  ...
  const edges = edgesData
    ...
    .map(e => ({
      ...
      rawData: {
        ...e,
        source_name: nodeMap[e.source_node_uuid]?.name || t('processGraph.unknown'),
        target_name: nodeMap[e.target_node_uuid]?.name || t('processGraph.unknown')
      }
    }))
